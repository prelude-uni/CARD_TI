...
function sanitizeExtraction(value: unknown): InformationExtractionResult {
  ...
  const statementValue = isRecord(value.statement) ? value.statement : {};
  ...
  return {
    statement: {
      billingMonth: safeString(statementValue.billingMonth),
    },
    transactions,
  };
}
...
function getMessageContent(responseBody: unknown): unknown {
  ...
  if ("statement" in responseBody || "transactions" in responseBody) {
    return responseBody;
  }
...
export async function extractInformation(
  ...
  return sanitizeExtraction(structuredContent);
}
